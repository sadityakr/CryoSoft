...
|---|---|
| `i2as://status` | State, active run, per-instrument readings and ramps, faults, holds, **attendance** and the **kill switch**. |
| `i2as://station` | Every configured instrument, what it reads, what it can be asked to do, within which bounds. |
| `i2as://manifest` | The same declaration with capabilities resolved into groups. |
...
1. **Read `i2as://status`.** Is a run already going? Is the state
...
2. **Read `i2as://station`** if you have not this session, so you are
...
6. **Watch it.** State changes and status snapshots arrive on their own as
   ...
   Read `i2as://status` when you need the full picture.
...
make the station safe). It appears in `i2as://status`. If it changes
