...
separate process (`python -m cryosoft.analysis run --spec <file>`) that can
...
Orchestrator ── run finished ──▶ session/eln publisher
                        ...
                        session/analysis_runner (QProcess)
                                     │  spec.json
                                     ▼
                  python -m cryosoft.analysis run --spec …      ← this package
                                     │  report.json + figures
                                     ▼
...
- **matplotlib is the optional `analysis` extra**, imported lazily with the
  ...
  naming `pip install cryosoft[analysis]`, and a well-behaved recipe turns
...
2. **A new recipe for one experiment** is `python -m cryosoft.analysis
