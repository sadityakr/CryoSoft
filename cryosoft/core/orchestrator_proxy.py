...
class OrchestratorProxy(QObject):
    ...
    def _forward(self, name: ev.CommandName, **kwargs: Any) -> str:
        ...
        resolved through its run catalog); this path exists only until the
        queue holds specs and the engine builds every run itself, at which
        point these four join the rest.
