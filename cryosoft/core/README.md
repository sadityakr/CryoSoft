...
- **The seam** (GLOSSARY.md's **Instrument host** and **Orchestrator proxy**).
  ...
  In `inline` mode this is exactly the previous construction with the thread
  decision given a name and a home, so behaviour is identical and the GUI
  suite is the proof.

- **The instrument thread** (GLOSSARY.md's **Instrument thread**), the host's
  second mode. `InstrumentHost(mode="threaded")` runs the station factory and
