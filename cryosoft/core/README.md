...
- **The status-mirror standard** (GLOSSARY.md's **Status mirror**). A client
  never reads the engine: it holds a mirror fed by the event stream and
  answers every read from the last `StatusSnapshot`, `StateChange` and
  `StationInfo` it saw. The stream is broadcast, though, and a client that
