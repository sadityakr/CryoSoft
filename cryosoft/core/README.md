...
| `procedure.py` | `BaseProcedure` — abstract base class defining the 4-method interface (`initiate`/`change_sweep_step`/`measure`/`standby`) plus `abort()`. `initiate()`/`standby()` return a `PhasePlan`, `change_sweep_step()` a `StepPlan \| None`, `abort()` a `tuple[Command, ...]` (default closes the data file and returns `()`; subclasses add measurement safe-off commands). The typed currency lives in `plan.py` |
