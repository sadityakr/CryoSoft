...
- `Orchestrator.submit(Command) -> request_id` is the **engine port**: the
  ...
  `run_builder.build_procedure()` assembles into a run; `RUN_OPERATION` /
  `QUEUE_OPERATION` carry `operation` plus `params` and are built as
  `cls(station, **params)`; `SET_EXPERIMENT_ENVELOPE` carries the envelope's
...
- **The seam** (GLOSSARY.md's **Instrument host** and **Orchestrator proxy**).
  ...
  In `inline` mode, the only mode that exists today, this is exactly the
