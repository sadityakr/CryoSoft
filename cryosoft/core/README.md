...
| `logging_config.py` | Configures the rotating file + console handlers and four time-rotated JSONL streams — `cryosoft.status` (`status.jsonl`, daily/UTC, 7 backups) and the tiered trend-history streams `cryosoft.trend_raw`/`trend_3min`/`trend_hourly` (`trend_history_{raw,3min,hourly}.jsonl`, daily/daily/weekly, all UTC, 2/8/53 backups); resolves the log directory (`CRYOSOFT_LOG_DIR` env var, then the platform user-data location, then `cryosoft/logs/`) | `setup_logging()`, `log_directory()` | `test_logging_config.py` |
| `tiered_trend_logger.py` | Qt-free live incremental writer for the tiered trend-history store (docs/plans/trend-history-persistence.md §2): one throttled raw-tier JSONL line per `record()` call plus two independent bucket accumulators (3-min, 1-hour) flushed to their own JSONL streams on a bucket-boundary crossing. Not yet wired into the Orchestrator tick (Phase 3) | `TieredTrendLogger` (`record(flat, timestamp, orch_state=None)`) | `test_tiered_trend_logger.py` |
| `trend_history.py` | Qt-free read/query side of the tiered trend-history store (docs/plans/trend-history-persistence.md §3): merges each tier's live + rotated JSONL files (excluding sync-conflict-copy filenames), and turns them into plottable series (GUI) or agent-facing evidence — count-weighted aggregate recombination, `persisted` flag distinguishing "no data in window" from "never persisted" (measurement-VI keys, plan §7), threshold-crossing detection | `TIERS`, `TierSpec`, `KeySummary`, `pick_tier()`, `persisted_keys()`, `read_tier()`, `read_window()`, `summarize()`, `find_crossings()` | `test_trend_history.py` |
