...
| `trend_checks.py` | The **Trend check** standard's pure policy core: `TrendCheck` declarations evaluated against `trend_history.summarize()` (never re-deriving mean/std or re-picking a tier), a `CheckResult` verdict whose `passed` is `True`/`False`/`None` (`None` = no data in the window, distinct from a definite fail), and the `Condition`-publication adapter for a definite failure. Qt-free, Station-free, import-linter contract C15 | `TrendCheck`, `CheckOutcome`, `CheckResult`, `Predicate`, `run_check()`, `run_checks()`, `no_data_outcome()`, `to_condition()`, `conditions_for()`, `declared_checks()` | `test_trend_checks.py` |
