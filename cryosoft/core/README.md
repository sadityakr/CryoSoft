...
| `plan.py` | Typed vocabulary of frozen dataclasses shared across every layer | `Target`, `Command`, `PhasePlan`, `StepPlan`, `ParamSpec`, `ParamGroup`, `ReadingVariant` (reading-loop currency), `DataSchema` (`.multiplexed()` per-label suffixing — routes and loop index labels, `.validate()` raising `DataSchemaError`) | `test_plan.py` |
...
| `orchestrator.py` | L3 single-threaded cooperative state machine; the sole hardware writer; runs the monitor + status cycle each tick inside an exception boundary that degrades to ERROR | `Orchestrator` (`run_procedure`, `queue_procedure`, `run_queue`, `pause_procedure`, `resume_procedure`, `abort_procedure`, `recover_from_error`, `acknowledge_emergency`, `submit_vi_action`, `submit_global_action`, `get_operational_status`); `OrchestratorState` enum | `test_l3_orchestrator.py` |
| `procedure.py` | L4 base classes: the Orchestrator-driven lifecycle and the generic sweep engine | `BaseProcedure` (`initiate` -> `PhasePlan`, `change_sweep_step` -> `StepPlan \| None`, `measure`, `standby` -> `PhasePlan`, `abort` -> `tuple[Command, ...]`, `get_param_groups()` classmethod); `SweepMeasureProcedure` (GUI-selected measurement VI, mux route selection, the reading loop — routes x a user-entered value list for one `reading_setters` parameter per datapoint, `L{i}` column labels + HDF5 `loop_labels` metadata, `DataSchema` assembly; concrete axes supply six hooks) | `test_l4_procedure.py`, `test_new_procedures.py` |
