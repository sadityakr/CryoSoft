...
- The **run-ownership standard** (GLOSSARY.md's **Run owner** and
  **Takeover**; full text in `Orchestrator._run_owner_admission()`): the
  ...
  dropped the moment the run ends. Four commands are owner-scoped for agents
  — `OWNER_SCOPED_COMMANDS`: `abort_procedure`, `confirm_operation`,
  `skip_operation_step`, `finish_operation`, the ones that end somebody's
  result or attest to a physical step of it. An `agent` that is not the owner
