...
- The GUI (or a future cryogenics panel) submits an `OperationBase` instance to
  `Orchestrator.run_operation()` / `queue_operation()` — a second, higher-
  priority request type driven by the same tick loop and state machine as
  `run_procedure()`/`queue_procedure()`.
