...
- **One vocabulary for live and stored runs** (`data_reader.py`'s module
  ...
  in flight. `RunSource` is that vocabulary as a `Protocol`, and every
  implementation shares one `summarise_values()` so two sources of the same
  run cannot disagree about what "mean" means. A consumer — the GUI's live
  view today, an agent gateway later — depends on the protocol, never on
  which source it holds. The reader is standalone by contract C16 (only
