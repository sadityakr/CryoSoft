"""cryosoft.mcp — the **MCP adapter**: a translator, in its own process.

Run as ``python -m cryosoft.mcp``, this package speaks the Model Context
...
contract C21 in ``pyproject.toml`` is what makes that true: the only CryoSoft
module this package may import is ``cryosoft.core.events``, the pure
...
See ``cryosoft/mcp/README.md`` for the folder standard and ``GLOSSARY.md``
...
from cryosoft.mcp.translate import (
