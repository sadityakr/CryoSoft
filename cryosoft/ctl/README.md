...
| `discovery.py` | The run catalog this entry point hands down: import every module of `cryosoft.procedures` and `cryosoft.procedures.operations`, then catalog every named subclass by class name. One broken module is logged and skipped rather than leaving the client with no catalog. | `discover_run_catalog()` | `tests/test_ctl.py` |
