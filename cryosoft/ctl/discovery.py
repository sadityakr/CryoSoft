...
``cryosoft.procedures``: a run travels as a class NAME, and whoever owns
discovery resolves that name and hands the catalog down. The application's
entry point does exactly this before it builds anything; ``cryosoft.ctl`` is
the second entry point, so it does it too, and contract C20 names
``cryosoft.procedures`` as the one layer above ``core``/``session`` this
