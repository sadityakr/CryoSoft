...
#   Stores data and station snapshots in an HDF5 file via DataManager.
...
#     iv_measurement (measurement VI with configure() and read_datapoint()).
# input: |
...
#   current, n_readings, init_wait, step_wait.
# process: |
...
class FieldSweepIV(BaseProcedure):
    ...
    1. ``initiate()``: ramp magnet_x to first field, temperature_vti to target T,
       configure delta-mode on iv_measurement. Create DataManager.
    2. ``measure()``: read IV data from iv_measurement, snapshot station state,
    ...
    def _build_sweep_array(self) -> list:
        """Build field sweep array from field_start, field_end, field_steps.
    ...
    def measure(self) -> None:
        ...
        Reads from ``iv_measurement.read_datapoint()`` (must have been
        ...
        # Read measured data from the measurement VI.
        measured_data: dict = self._station.iv_measurement.read_datapoint()

        # Add sweep column: actual field at time of measurement.
        measured_data["field_T"] = self._station.magnet_x.get_field()

        # Snapshot full station state.
        snapshot = self._station.get_state()
