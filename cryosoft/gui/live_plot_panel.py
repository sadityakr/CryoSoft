# ---
# description: |
#   LivePlotPanel: a self-contained live-plot widget (QGroupBox) with an X-axis
#   selector, a Y-axis selector, and a themed pyqtgraph PlotWidget. Extracted from
#   ProcedureWindow, which previously duplicated Plot 1 / Plot 2 almost verbatim.
#   Each panel owns its selectors and curve, repopulates its axis choices from a
#   key list, and redraws itself from a datapoint history it is handed.
...
#   a list of available axis keys via set_available_keys(), and a datapoint
#   history (list of enriched dicts) via redraw().
# process: |
#   set_available_keys() repopulates both selectors, preserving a still-valid
#   selection. redraw() extracts scalar X/Y series from the datapoint history and
#   feeds them to the curve. Changing either selector redraws against the last
#   datapoint list the panel was handed.
# output: |
...
class LivePlotPanel(QGroupBox):
    """A live X/Y plot panel: two axis selectors plus a themed pyqtgraph curve.
    ...
    the parent re-supplying the data.
    ...
    def __init__(
        ...
        x_row.addWidget(self._x_selector)
        x_row.addStretch()
        ...
        y_row.addWidget(self._y_selector)
        y_row.addStretch()
    ...
    def _redraw(self) -> None:
        ...
        xs = [_extract_scalar(dp.get(x_key)) for dp in self._datapoints]
        ys = [_extract_scalar(dp.get(y_key)) for dp in self._datapoints]
