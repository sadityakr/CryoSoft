# ---
# description: |
#   RampTrackerPanel: MonitorWindow page 1's bottom-right quadrant, left
#   sub-panel. One generic RampRow per ramp running right now — current
#   value, NEXT setpoint, END setpoint, rate, sub-phase, the run that owns
#   it (if any) — plus a per-ramp Abort button. Answers the question the
#   Monitor window could not before: a field or temperature ramp started
#   from an instrument card was invisible once running and could only be
#   stopped by aborting everything. Contains no per-instrument logic: rows
#   are built entirely from the RampRecord list the Orchestrator publishes,
#   so a new rampable VI appears here the moment it implements the
#   ramp-introspection standard (RampableVI's ramp_value/ramp_setpoint/
#   ramp_target/ramp_rate/ramp_phase hooks) — this file never changes.
# entry_point: Not run directly. Built by MonitorWindow._build_ramps_quadrant.
# dependencies:
#   - PyQt6 >= 6.5
#   - qtawesome
#   - cryosoft.core.orchestrator (Orchestrator — stop_ramp + ramps_updated)
#   - cryosoft.core.ramps (RampRecord — the value object rows render)
# input: |
#   One list[RampRecord] per on_ramps_updated() call, forwarded by
#   MonitorWindow from Orchestrator.ramps_updated (which fires every tick,
#   hence the window-as-receiver forwarding — see monitor_window.py's
#   teardown-race note).
# process: |
#   on_ramps_updated() reconciles the row set against the records by VI
#   name: a new ramp gets a row, a finished one has its row removed, and a
#   surviving one is updated in place (no rebuild, so nothing flickers and
#   objectNames stay stable across ticks). Each row's Abort button confirms
#   via a QMessageBox and then calls orchestrator.stop_ramp(vi_name); it is
#   disabled, with the refusal as its tooltip, whenever the record says the
#   stop would not be admitted — which is exactly the case where a run owns
#   the ramp, since the record's stoppable flag comes from the same
#   admission predicate stop_ramp() itself uses.
# output: |
#   A QWidget hosted (scrolled) in MonitorWindow's bottom-right quadrant.
#   Side effect: submits a single-VI ramp stop to the Orchestrator.
