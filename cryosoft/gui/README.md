...
package (`cryosoft.analysis.report` for the **Analysis report** the eLab tab
previews, and `cryosoft.analysis.discovery` for the recipe catalogue, imported
...
`LogPanel`. Page 1's bottom-right quadrant is a VERTICAL splitter
(`agents_splitter`) of two levels. On top, a horizontal splitter
(`bottom_right_splitter`) of two sub-panels: `RampTrackerPanel` on the left
(always built — every ramp running right now, each with its own Abort) and
the optional `OperationsPanel` on the right (present when cryogenics is
enabled OR an `operations:` config block is declared; a placeholder label
otherwise). Underneath, across the full width, the `AgentPanel` (always
built) — full-width because each of its rows is one wide line that a third
column would only wrap. The header carries the `TakeoverStrip` (kill switch,
...
- A `Station` instance: the registered VI names and each VI's `vi_type`
  ...
  and operations are constructed against. No widget holds a VI: the
...
- An `OrchestratorProxy` (`core/orchestrator_proxy.py`), built by the
  **Instrument host** and handed in where the `Orchestrator` itself used to
  ...
  signals are routed by run kind (**Hard status separation**, see
  GLOSSARY.md):
  ...
  `StatusMirror.active_run_kind()`). Actions the GUI
...
- Optionally, the L6 cryogenics stack (**Cryogenics log** and **Servicing
  log** in GLOSSARY.md):
...
- Optionally, a resolved `operations:` config block (`Station.
  read_operations_config()`, `operations_config` keyword param) —
...
- **`param_form.py` is the single ParamSpec-to-Qt-widget mapping.** It is the
  ...
  a widget.
- **`assistant_dock.py` renders the assistant; it never drives it.** The dock
  holds no engine, builds no `Gateway` and calls no tool. Every line it shows
  is a transcript record the `AssistantRuntime` published — the same record
  written to the **Assistant transcript** — so what is on screen and what is in
  the evidence file cannot drift apart, and the role selector asks a factory
  the application handed it rather than constructing a connection of its own.
  Its cost line is the runtime's two **cost line**s rendered, never recomputed.
  The **assistant's thread rule** (`session/assistant/README.md`) is what keeps
  this dock from breaking the no-blocking rule above: the model call is on a
  pool thread and its answer arrives as a queued signal, so nothing here waits
...
| `assistant_dock.py` | The **Embedded assistant**'s chat dock (`assistant_dock`): transcript view, question field, visible cost line (this turn / this session), a role selector capped at the deployment's ceiling, a Stop button that cancels between steps, and a four-state status chip (idle / thinking / calling `<tool>` / refused). Renders one `AssistantRuntime` and holds nothing else; with no runtime it builds anyway and says "no API key configured" in one line. Registered by `MonitorWindow` only when the setup's `monitor.yaml` says `assistant: true`. | `AssistantDock`, `NO_CLIENT_MESSAGE` | `tests/test_assistant_dock.py` |
