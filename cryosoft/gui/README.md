...
`cryosoft.core.sweep_builder`; contract C8 forbids only drivers and concrete
VI subpackages, not `cryosoft.procedures`), L2 Station
...
1. Create a file in this folder. Import only from `PyQt6`, `cryosoft.core.*`
   ...
   `cryosoft.procedures.*` (never their VI/driver dependencies), and other `cryosoft.gui.*` widgets. Do not import drivers or
   call VIs directly.
