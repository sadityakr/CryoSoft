...
Depends on: Layer 3 (Orchestrator signals), Layer 4 (BaseProcedure.get_param_groups() returning `ParamGroup`/`ParamSpec` from `cryosoft.core.plan`), Layer 2 (Station.get_vi_names()).
...
2. Import only from `PyQt6`, `cryosoft.core.orchestrator`, `cryosoft.core.station`, `cryosoft.core.decorators`, and the typed value objects in `cryosoft.core.plan` (e.g. `ParamSpec`, `ParamGroup`).
...
| `param_form.py` | ParamSpec → Qt-widget mapping for the procedure parameter form — the ONE place that names widget classes for procedure parameters. `build_param_widget(name, spec)` picks the widget (`choices` → `QComboBox` showing labels / collecting the mapped value, `bool` → `QCheckBox`, else `QLineEdit` seeded with the default); `build_group_box(group)` / `build_form_layout(params)` lay them out as `QFormLayout` rows (label = `name (unit):`, description/default/range tooltip, objectName `param_{name}_input`); `collect_value` reverses the mapping; `get_widget_raw`/`set_widget_raw` give the string round-trip the session cache uses. L4 declares `ParamSpec`s and never names a widget; new input kinds are added here. |
| `procedure_window.py` | `ProcedureWindow(QMainWindow)` — procedure builder and live-data window. Auto-discovers `BaseProcedure` subclasses from `cryosoft.procedures.*`. Auto-generates parameter forms from each procedure's `ParamGroup`s (`BaseProcedure.get_param_groups()`), delegating widget construction to `param_form`; the Sweep column additionally hosts the `SweepAxisWidget`. Manages a local queue (mirrored into the Orchestrator via `queue_procedure`). Both the run-now and queue paths construct instances through the single `_build_procedure_instance` path. Two `LivePlotPanel`s (driven by `orchestrator.measurement_ready`) show live data. Progress bar driven by `orchestrator.procedure_progress`. Emergency-acknowledge button visible only in EMERGENCY state. |
