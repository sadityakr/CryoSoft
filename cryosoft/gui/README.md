...
| `session.py` | Qt-free persistent user-session model (sample metadata, data dir, last procedure + params, run queue), serialised to one JSON file; never raises on a corrupt file. | `SessionState`, `load`, `save` | `tests/test_session.py` |
...
| `live_plot_panel.py` | Reusable live X/Y plot panel (X + Y selectors, optional Route and Loop selectors for multiplexed/looped measurements, themed `pyqtgraph` curve); axis keys stay plain and the panel composes `{key}__L{i}__{route}` at draw time. ProcedureWindow hosts two, driven by `measurement_ready`. | `LivePlotPanel`, `set_available_keys`, `set_available_routes`, `set_available_loop_labels`, `redraw`, `clear` | `tests/test_gui.py` (via ProcedureWindow `_plot1`/`_plot2`) |
