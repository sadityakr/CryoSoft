...
| `sample_info_panel.py` | The Sample Info quadrant: an experiment status/Start-Close control (SessionManager, optional) plus sample name/ID/comments and the data-directory field with Browse. Sample fields stay free-editable per run regardless of experiment state; whatever they hold at "Start Experiment" time is snapshotted onto the `ExperimentRecord`. Single GUI owner of session-level sample metadata, surfaced to ProcedureWindow through MonitorWindow's accessors. | `SampleInfoPanel`, `get_sample_info`, `get_data_dir`, `apply_session` | `tests/test_gui.py` |
| `experiment_dialogs.py` | Modal dialogs for the experiment lifecycle: `StartExperimentDialog` (title, user picker with inline "New user…", attendance checkbox) and `CloseExperimentDialog` (findings text). Opened only by `SampleInfoPanel`; every `SessionManager` mutation happens in the panel after a dialog accepts. | `StartExperimentDialog`, `CloseExperimentDialog`, `AddUserDialog` | `tests/test_gui.py` |
