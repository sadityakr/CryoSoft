...
- **`operations_panel.py`'s `OperationCard` is the single per-operation card
  ...
  detail (checklist rows, next-due line, ready banner, confirmations)
  renders generically from an `OperationBase` instance's declarations
  (`readiness_conditions()`, `next_due()`, `ready_message`,
  `operator_confirmations`) plus a panel-supplied factory closure for
  building a fresh run instance. Adding an operation to a setup never
  touches this file; a new operation with none of those declarations still
  gets a working card (just a bare button).
...
| `form_autosave.py` | Qt-free form-autosave model (sample metadata, data dir, last procedure + params, run queue, live-plot panels' X/Y/Loop axis selections), serialised to one JSON file; never raises on a corrupt file. Historically `session.py` — renamed so "session" is free for the L6 Session Management layer; classes and the JSON file keep their old names for compatibility. | `SessionState`, `load`, `save` | `tests/test_form_autosave.py` |
...
| `live_plot_panel.py` | Reusable live X/Y plot panel (X + Y selectors, optional per-slot Loop 1 / Loop 2 selectors for looped measurements, themed `pyqtgraph` curve); axis keys stay plain and the panel composes `{key}__A{i}__B{j}` at draw time. ProcedureWindow hosts two, driven by `measurement_ready`. `export_selection`/`restore_selection` round-trip the X/Y/Loop choices through `form_autosave.SessionState.plot_selections`. | `LivePlotPanel`, `set_available_keys`, `set_available_loop_labels`, `redraw`, `clear`, `export_selection`, `restore_selection` | `tests/test_gui.py` (via ProcedureWindow `_plot1`/`_plot2`) |
...
| `procedure_window.py` | Procedure builder, run queue, and live-data window — a composition shell over `ProcedureParamsPanel`, `QueuePanel`, and two `LivePlotPanel`s. Same 2x2 splitter grid (params / queue-over-status / Plot 1 / Plot 2); single procedure-construction path shared by Run Now and the queue; progress bar from `procedure_progress`; emergency-acknowledge button visible only in EMERGENCY. `export_session_state`/`_restore_session` include both plots' axis/loop choices; restore re-refreshes the plots' key lists from the params panel's settled selection first, since `ProcedureParamsPanel.restore_session` applies cached structural values (e.g. a non-default measurement VI) with signals blocked. | `ProcedureWindow` | `tests/test_gui.py` |
