...
class OxfordILM210:
    ...
    Exposes the same public API as SimOxfordILM210.
    """
    ...
    def __init__(self, resource_string: str) -> None:
        ...
        try:
            ...
            raise CryoSoftCommunicationError(
                f"Cannot open ILM 210 at {resource_string}: {exc}",
                vi_name="OxfordILM210",
            ) from exc

        self._instr.timeout = 3_000
        # ILM210 serial settings
        ...
        # Two stop bits — required by ILM protocol
    ...
    # ------------------------------------------------------------------
    # Public API (matches SimOxfordILM210)
    # ------------------------------------------------------------------
    ...
    def get_helium_level(self) -> float:
        ...
        try:
            ...
            raise CryoSoftCommunicationError(
                f"ILM210: cannot parse helium level from {resp!r}: {exc}",
                vi_name="OxfordILM210",
            ) from exc
    ...
    def get_nitrogen_level(self) -> float:
        ...
        try:
            ...
            raise CryoSoftCommunicationError(
                f"ILM210: cannot parse nitrogen level from {resp!r}: {exc}",
                vi_name="OxfordILM210",
            ) from exc
    ...
    def close(self) -> None:
        ...
        (local, unlocked) is what returns control to the ILM 210's own front
    ...
    def _set_remote(self, mode: int) -> None:
        ...
        try:
            ...
            log.warning("ILM210: could not set remote mode %d: %s", mode, exc)

    def _execute(self, cmd: str) -> str:
        """Write an ISOBUS command and return the response.

        Prefixes the command with ``@{number}`` and waits 70 ms.
