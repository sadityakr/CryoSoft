...
report directory and launches ``python -m cryosoft.analysis run --spec
...
from cryosoft.session.eln.drafting import manifest_from_run
...
class AnalysisRunner(QObject):
    ...
    def _start_next(self) -> None:
        ...
        process.start(
            self._python,
            ["-m", "cryosoft.analysis", "run", "--spec", str(request.spec_path)],
        )
