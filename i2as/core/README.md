...
- `Orchestrator` calls `build_operational_status()` then `apply_stall_verdict()`
  ...
  **The record standard** — every field, its type, and the schema version —
  ...
  for its readers (`troubleshoot/status_reader.py`), so a field is only ever added, never renamed,
  ...
  repurposed. (`CRITICAL_FLAG` names the DECLARED severity of whatever
  tripped rather than one magnet's status word, so any setup's critical
  flag reaches it.)
...
- `Orchestrator.submit(Command) -> request_id` is the **engine port**: the
  ...
  because the Orchestrator may not import `i2as.procedures` (contract C5);
...
- **The instrument thread** (GLOSSARY.md's **Instrument thread**), the host's
  ...
  overridable for one launch by `I2AS_INSTRUMENT_THREAD`, resolved by
...
- **`inline` is temporary** (GLOSSARY.md's **Inline mode**). `threaded` is the
  ...
  back is `I2AS_INSTRUMENT_THREAD=0` for one launch or
...
- `Orchestrator.vi_faults()` / `acknowledge_fault()` / `retry_fault()` are the
  ...
  **Instrument fault**) — the RUNTIME sibling of `offline_vi_names()` /
  ...
  disconnect: a fault means the instrument stopped answering while I2AS
...
- `Orchestrator.availability(vi_name)` / `availabilities()` are the single
  GUI-facing accessor for the Availability standard (`i2as.core.
...
- The capability-scope standard: every `@control` method carries a scope
  ...
  `I2ASSafetyError` before anything is dispatched. The Orchestrator passes
...
- The **direct action path** standard (GLOSSARY.md's **Direct action path**;
  ...
  underscore-prefixed name (`I2ASPrivateActionError`), a method that is
  neither `@control` nor one of `LIFECYCLE_ACTIONS` (`initiate`/`standby`,
  `I2ASUndeclaredActionError`), a capability whose scope exceeds the
  caller's `allowed_scope` (`I2ASActionScopeError` — the default is the
...
- The **run-ownership standard** (GLOSSARY.md's **Run owner** and
  **Takeover**; full text in `Orchestrator._run_owner_admission()`): the
  ...
  emergency standby, `stop_ramp`, pause and resume hold the station rather
...
- The **verdict standard** (GLOSSARY.md's **Verdict standard**; full text in
  ...
  parsing prose, and a `I2ASSafetyError`'s structured
...
- The **System-Condition standard** (GLOSSARY.md's **System condition** /
  **Severity ladder**; full text in `core/conditions.py`'s module
  docstring): every "something is wrong" signal in I2AS — a VI's
...
| `conditions.py` | The System-Condition standard's pure policy core (origin × severity, scope follows severity): the `Condition`/`Verdict` value objects and the deterministic `decide()` verdict function. Stdlib-only — no other `i2as` import, machine-enforced by import-linter contract C13 | `Condition`, `Verdict`, `decide()`, `envelope_conditions()`, `SEVERITIES`, `ORIGINS` | `test_conditions.py` |
| `availability.py` | The Availability standard's pure policy core (GLOSSARY.md's **Availability** / **Availability tag**): a closed tag vocabulary, a declared per-tag policy table, and the deterministic `state_for()`/`decide_availability()` functions that turn a VI's tag set into one of four mutually exclusive states plus a `TagPolicy` verdict. Stdlib-only — no other `i2as` import, machine-enforced by import-linter contract C14, mirroring C13's isolation of `conditions.py` above | `AVAILABILITY_TAGS`, `AVAILABILITY_STATES`, `TAG_PRECEDENCE`, `TagPolicy`, `TAG_POLICY`, `Availability`, `state_for()`, `decide_availability()` | `test_availability.py` |
| `exceptions.py` | The exception hierarchy every layer catches by subtype, including the direct action path's typed refusals (each admission check has its own subclass, so a caller tells them apart without parsing prose) | `I2ASError`, `I2ASCommunicationError`, `I2ASSafetyError`, `I2ASActionRefusedError`, `I2ASPrivateActionError`, `I2ASUndeclaredActionError`, `I2ASActionScopeError`, `I2ASConfigError`, `DataSchemaError` | `test_foundation.py`, `test_direct_action_path.py` |
...
| `capability_manifest.py` | The **capability manifest**: the JSON rendering of `Station.station_info()`, resolving each instrument's capabilities into its declared UI groups (declared order, ungrouped items after) and adding no fact of its own. `MANIFEST_SCHEMA` is a real JSON Schema draft 2020-12 document; `validate_manifest()` checks against it with a small structural validator, since `jsonschema` is deliberately not a project dependency. `python -m i2as.core.capability_manifest <config_dir>` prints one, building a Station and nothing else — no `QApplication` | `MANIFEST_SCHEMA`, `MANIFEST_SCHEMA_ID`, `build_manifest()`, `validate_manifest()`, `main()` | `test_capability_manifest.py`, `test_conformance.py` |
...
| `paths.py` | Resolves machine-local, per-installation directories and settings outside source control. The **per-user path standard**: two per-user roots, `user_config_dir()` (what the user chooses and would carry along — editable config copies, ELN settings; `%APPDATA%\I2AS`, else `$XDG_CONFIG_HOME/i2as`, else `~/.config/i2as`) and `user_state_dir()` (what the app accumulates here — logs, trend history, the gateway descriptor; `%LOCALAPPDATA%\I2AS`, else `$XDG_STATE_HOME/i2as`, else `~/.local/state/i2as`), every per-user file hanging off one of them: the log directory is `I2AS_LOG_DIR`, else `user_state_dir()/logs`; `session/eln/settings.py` and `troubleshoot/cli.py` call the config root (the MCP adapter keeps a copy of the state rule under contract C21). Also the fixed measurement root (`I2AS_MEASUREMENT_ROOT` env var, else the `measurement_root` key in the machine-level `App-config.yaml` settings file, else refuse to start — see GLOSSARY.md's **Measurement root**). Stdlib-only, import-linter contract C1 foundation module | `user_config_dir()`, `user_state_dir()`, `log_directory()`, `measurement_root()` | `test_paths.py` |
| `logging_config.py` | Configures the rotating file + console handlers and four time-rotated JSONL streams — `i2as.status` (`status.jsonl`, daily/UTC, 7 backups) and the tiered trend-history streams `i2as.trend_raw`/`trend_3min`/`trend_hourly` (`trend_history_{raw,3min,hourly}.jsonl`, daily/daily/weekly, all UTC, 2/8/53 backups); log directory resolution delegates to `paths.log_directory()` | `setup_logging()` | `test_logging_config.py` |
