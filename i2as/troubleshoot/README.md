...
## Architecture layer
None of L0–L5. Like `i2as/main.py`, this package sits *beside* the stack
as a leaf entry point: it may import drivers (L0) and the Station's config
helpers (L2), and nothing in i2as imports it. Contracts C9/C10 in
...
entry point `python -m i2as.ctl` (`i2as/ctl/`, the **Reference
...
satisfiable at all. Anything Qt belongs in `i2as/gui/`, not here.

## CLI
`python -m i2as.troubleshoot <subcommand>` — one-shot commands, each
...
(`i2as.core.paths.log_directory()`, overridable via `I2AS_LOG_DIR`)
...
- `status_reader.py` — the runtime half: parses `status.jsonl` (the
  ...
  standard lives in `i2as/core/operational_status.py`'s module
  docstring), folds a window of records into a digest, and renders it in
  plain English. Depends on the JSONL shape only, never on `i2as.core`
...
- `session_report.py` — the after-the-fact half: reads one experiment
  ...
  `i2as.session`, so it parses the record files directly from the shape
  `i2as/session/models.py` documents and the layout
  `i2as/session/store.py` writes — those two files are its contract, and
...
- `__main__.py` — `python -m i2as.troubleshoot` entry point.
