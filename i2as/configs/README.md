...
## Architecture layer
L2 input — consumed by the Station factory (`i2as/core/`, see
`build_station`). Configs contain no Python; they are declarative YAML data.

## Entry (what comes in)
A config is selected by directory path (e.g. `i2as/configs/sim_cryostat`).
...
- `real_drivers:` — map of instance name → `{class: <dotted driver path>,
  address: <VISA resource string>}`. Real configs use `i2as.drivers.<real>`;
...
`I2AS_INSTRUMENT_THREAD=0` (or `=1`) overrides this file for one launch,
...
write-up from the VI side is `i2as/virtual_instruments/README.md`
("GUI presentation").
...
`python -m i2as.troubleshoot trends`.
...
pair of config directories: one naming the real `i2as.drivers.*` classes at
