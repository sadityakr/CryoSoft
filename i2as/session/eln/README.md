# i2as/session/eln — ELN publishing (L6)
...
**L6, inside the Session Manager.** Imports `i2as.session.*` and
`i2as.core.*` downward and nothing else — the same C11/C12 import
contracts that bind the rest of `i2as/session/` apply here unchanged. It
...
- **Settings**: the user-level JSON file resolved by `eln_settings_path()`
  (`$I2AS_ELN_SETTINGS`, else `eln-settings.json` under
  `core.paths.user_config_dir()` — `%APPDATA%\I2AS` on Windows,
  `~/.config/i2as` elsewhere), plus the
  `I2AS_ELAB_APIKEY` environment variable which overrides its `api_key`.
...
- **A drain tick**: `ElnPublisher.drain_once()`, called by the publisher's own
  QTimer, started from `i2as.main` once the application has an event loop.
- **An analysis report** (`i2as.analysis.report.AnalysisReport`, or its
...
- **`ElnEntryRef`** — backend, entry id, URL, template id — the value a
  ...
  `i2as.session.models.ElnLink` so a persisted link is
...
- **The API keys are never logged.** `ElnSettings` and `AssistantSettings`
  ...
  `assistant.api_key`, or from `I2AS_ASSISTANT_APIKEY`; an empty key
