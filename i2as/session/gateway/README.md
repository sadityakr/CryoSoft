...
**MCP adapter** (`i2as/mcp/`, a SEPARATE process with its own README),
...
**L6, inside the Session Manager.** Imports `i2as.core.*` and
`i2as.session.*` downward and nothing else — bound by the same
import-linter contracts as the rest of `i2as/session/`: **C11** (session
...
`python -m i2as.analysis` worker gives a human's own recipe.
...
6. **A new protocol is an adapter in its own process, not a module here.**
   ...
   another protocol translates into it from outside, the way `i2as/mcp/`
