...
`i2as.core.procedure.SweepMeasureProcedure`; the files here supply only the
axis specifics.
...
typed plan objects from `i2as.core.plan` (never bare dicts/tuples):
...
Every procedure subclasses `BaseProcedure` (from `i2as.core.procedure`); an
...
- Parameters are declared as `ParamSpec` value objects (from
  `i2as.core.plan`), grouped into `sweep_parameters` / `system_parameters` /
  ...
  ParamSpec -> Qt-widget mapping lives entirely in `i2as.gui.param_form`.
...
candidate refuses the run with `I2ASConfigError` (the message names the
