...
## Architecture layer
L1 — Virtual Instruments. Depends downward on `i2as.drivers` and
`i2as.core` (exceptions, decorators, plan). VIs never import Station,
...
- `@control` action methods, validated against `control_limits` before any
  hardware call (out-of-range raises `I2ASSafetyError`), carrying a
...
- **The setup config decides the ACTUAL card.** A `panels:` entry in the
  ...
  never VI code. See `i2as/configs/README.md` for the block's shape.
...
- The capability-scope standard: `@control` (bare, or `@control(scope=...)`)
  ...
  allowed_scope=...)` (`i2as.core.station`) — this folder only declares
...
- `base.py` — `BaseVirtualInstrument` plus the typed sub-bases `MagnetBase`,
  ...
  themselves are defined in `i2as.core.decorators`.) tests:
