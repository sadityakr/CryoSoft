...
**A leaf entry point, above everything.** Like `i2as.main` and
`i2as.troubleshoot`, nothing in `i2as` imports it. It imports
`i2as.core.*`, `i2as.session.*` and `i2as.troubleshoot.*`
downward, plus `i2as.procedures` for the one job an entry point owns:
...
which is exactly what `i2as.main` does before it builds anything.
Contract **C20** freezes the rest of the boundary: no GUI, no drivers, no
virtual instruments, ever.

## Entry (what comes in)

- **A command line**, in the grammar `python -m i2as.ctl [connection
...
2. Keep the dependency direction: `core`, `session`, `troubleshoot` and
   ...
   the build otherwise, and nothing in `i2as` may import this package.
...
|------|----------------|----------------|-------------|
| `__main__.py` | The entry point `python -m i2as.ctl` runs. Nothing but `sys.exit(main())`. | — | `tests/test_ctl.py` |
...
| `discovery.py` | The run catalog this entry point hands down: import every module of `i2as.procedures`, then catalog every named subclass by class name. One broken module is logged and skipped rather than leaving the client with no catalog. | `discover_run_catalog()` | `tests/test_ctl.py` |
