...
`agent-native-architecture.md` (§8) and `agentic-operation-roadmap.md`
...
`agent-native-architecture.md` sequences the MCP gateway first (A1
...
`agentic-operation-roadmap.md` sequences driver observability first, which
...
3. **Shared-instrument conformance test.** Precisely the test proposed in
   `agentic-operation-roadmap.md` §2b: pollute a sim instrument with stale
...
4. **`troubleshoot session` mode.** A sequence of commands executed in one
   ...
   (`agentic-operation-roadmap.md` §1b). Every CLI subcommand today opens
...
1. **Roles and action classes**: `observer` / `debug` / `session`, with the
   ...
   `agent-native-architecture.md` §3.2. Enforced server-side in the
...
- **Instrument transaction context manager with snapshot/rollback**
  (`agentic-operation-roadmap.md` §3a). Phase 3's `safe_shutdown()` plus
...
- **Driver proxy with structured diagnostic events**
  (`agentic-operation-roadmap.md` §3b). Largely subsumed by Phase 2's
...
- **Remote (non-localhost) gateway access, remote notifications, headless
  station mode.**
- **eLab publishing track** (`agent-native-architecture.md` Track B).
  Genuinely independent; sequence it against this plan by preference, not
  dependency.
