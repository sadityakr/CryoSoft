...
> **ARCHIVED 2026-07-25 — IMPLEMENTED.** Shipped as the session record,
> `gui_state.json`, and the derived Data Directory. Kept as the normative
> reference for the record format (`schema_version`, bundle-relative data
> paths, save-failure surfacing) and cited 6 times from shipped code.
> Restored from commit `70c910a^` after being deleted by accident in
> `70c910a`. For current planning see
...
This is directly the "records & store" foundation `docs/plans/archive/session-management-layer.md`
already called Phase 1 for, and the substrate `docs/plans/archive/agent-native-architecture.md`'s
planned Agent Gateway (`list_experiments()/get_experiment()/read_run_data()`)
is meant to read from. `docs/plans/archive/session-handling-architecture.md` is the
