...
> **ARCHIVED 2026-07-25 — IMPLEMENTED.** Operations (`core/operation.py`),
> the Servicing Log framework (`session/servicing_log.py`), and cryogenics
> management all shipped; the `**Status:** proposal` line below is stale and
> preserved only as the original text. This document is the **normative
> reference for the operation and servicing-log design** and is cited 38
> times from shipped code and tests, so it must not be moved or deleted
> again. Restored from commit `70c910a^` after being deleted by accident in
