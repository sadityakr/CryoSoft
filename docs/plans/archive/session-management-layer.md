...
> **ARCHIVED 2026-07-25 — PARTLY IMPLEMENTED, PARTLY STILL LIVE.** The
> record model, `ExperimentStore`, and `SessionManager` shipped
> (`cryosoft/session/`); the `**Status:** proposal` line below is stale for
> that part. **The eLab/ELN track (Track B) is genuinely unbuilt and is not
> covered by the framework roadmap** — there is no `cryosoft/session/eln/`,
> and `ElnLink` (`session/models.py:210-245`) is unwired scaffolding. This
> document remains the only design record for that work. Restored from
> commit `70c910a^` after being deleted by accident in `70c910a`. For the
> agent-facing roadmap see
