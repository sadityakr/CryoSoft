...
> **ARCHIVED 2026-07-25 — SUPERSEDED AS A ROADMAP, RETAINED AS DESIGN.**
> Its first slice shipped via `unified-session-record.md`; the hardening
> items it deliberately defers (store lock, migration harness, snapshot cap,
> sealing, the `runs/` escape hatch) are still unbuilt and still the right
> design should the session bundle need them. Roadmap sequencing is replaced
> by [`../agentic-instrumentation-framework.md`](../agentic-instrumentation-framework.md).
