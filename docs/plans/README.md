...
| [`control-ui-groups.md`](control-ui-groups.md) | **Intent brief, for planning** | What we want, why, and the shape of how for grouping a VI's `@monitored`/`@control` methods into titled boxes declared on the decorators (`UIGroup`, `group=` tag, `ui_groups`), rendered by the instrument panel and emitted by the capability manifest. Presentation and description only: nothing below the GUI changes. Takes the group primitive from `deferred/complete-instrument-vis.md`, leaves `@query` and mode exclusivity deferred. Input to a fuller refactor plan, not that plan. |
