...
|---|---|---|
| [`i2as-framework-extraction.md`](i2as-framework-extraction.md) | **Plan, awaiting approval (2026-09-05, branch `feature/i2as-framework`)** | Extracts the publishable framework, I2AS (Instrument to Agentic Station), from this repository: the four survey-backed keep/drop/generalise inventories per package, the seven generalisations that move cryostat judgements into the files a user writes (action class on the control, critical flag instead of quench, procedures that find instruments by role, config-declared trend checks), the trimmed sim cryostat that stays as the worked example, the eight phases with exit criteria, the rename plan with its runtime-literal classes, and six decisions for the user. |
