...
|---|---|---|
| [`i2as-framework-extraction.md`](i2as-framework-extraction.md) | **Approved 2026-09-05, in execution on branch `feature/i2as-framework`** | Extracts the publishable framework, I2AS (Instrument to Agentic Station), from this repository: the four survey-backed keep/drop/generalise inventories per package, the seven generalisations that move cryostat judgements into the files a user writes (action class on the control, critical flag instead of quench, procedures that find instruments by role, config-declared trend checks), the trimmed sim cryostat and the new sim imaging setup that stay as the two worked examples, the nine phases with exit criteria, the rename plan with its runtime-literal classes, and six decisions for the user. |
| [`development-plan-contracts-to-agents.md`](development-plan-contracts-to-agents.md) | **Implemented (2026-09-03, on the integration branch; A–G complete, real setups keep the thread flag off until a hardware soak)** | Sequences the work the framework, the instrument-thread plan, the UI-groups brief and the archived eLab design each own, contracts first: A contracts and declarations, B the seam, C the thread move, D results and safe state, E the agent gateway, F panel/MCP/assistant, G eLab. Names the owner document per step, states the assumptions taken from the open decisions, and gives a checkable milestone per phase. Adds no design of its own. |
| [`analysis-before-eln.md`](analysis-before-eln.md) | **Implemented (2026-09-05, on the integration branch)** | A dedicated analysis stage between a finished run and its eLab entry: the recipe contract (`cryosoft/analysis/`, package recipes plus per-experiment scripts), the out-of-process analysis worker, the analysed entry body (analysis output plus a short provenance block), the preview-then-approve publish gate in the procedure window's eLab tab, the in-app eLab setup dialog, and the agent tools that list, write, run and read analyses. Records the four decisions taken with the user. |
...
| [`control-ui-groups.md`](control-ui-groups.md) | **Implemented — `UIGroup`, `group=`, grouped front panels, manifest groups** | Lets a VI declare titled UI groups on `@monitored` / `@control` (`group=`, `UIGroup`, `ui_groups`), validated at class creation, rendered as titled boxes on the front panel and emitted by the capability manifest from the same metadata. Presentation and description only: nothing crosses the action queue. Also fixes the six `initiate_measurement` controls that declare no `params=`. Takes the group primitive from `deferred/complete-instrument-vis.md`; feeds the framework's Phase 1 manifest and the instrument-thread plan's `StationInfo`. Three decisions in its §6. |
