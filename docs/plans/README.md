...
| [`safety-hold-enforcement.md`](safety-hold-enforcement.md) | **Proposal, not started** | Makes a safety hold a maintained invariant instead of a one-shot command: a held VI is driven to standby and kept there for as long as the hold is active and unacknowledged. Adds a base-derived `standby_status()` (command provenance tracked by `__init_subclass__` wraps, zero VI-author burden), replaces the edge-triggered onset dispatch with level-triggered enforcement, announces every re-assertion, and escalates when the invariant cannot be met. Fixes the reported bug where a field set during an acknowledge window is never driven back down after the override expires. |
| [`trend-checks-temporal-analysis.md`](trend-checks-temporal-analysis.md) | **Proposal, not started** | The judgement layer over the tiered trend-history store: named, config-thresholded checks answering "has this been behaving over the last N hours" (temperature stability, helium consumption rate, store liveness) for an agent or a sleeping operator. Publishes failures on the Severity ladder's reserved `advisory` rung so there is one condition registry, not two. Phase 1 cleans and renames the existing stall detector (`core/watchdog.py` → `stall_detection.py`, deletes `STALLED_RUN`, fixes a threshold expressed in ticks against a per-setup tick interval). |
